...
if 'items' not in st.session_state:
    st.session_state.items = pd.DataFrame(columns=["Descripción", "Cantidad", "Precio Unitario", "Total"])
...
if clients_df.empty:
    ...
    with st.form("quote_form"):
        ...
        # --- Interactive Data Editor for Quote Items ---
        edited_items = st.data_editor(
            ...
            column_config={
                ...
                "Cantidad": st.column_config.NumberColumn(required=True, min_value=0, format="%d"),
        ...
        if 'data_editor' in st.session_state:
            edited_items = st.session_state['data_editor']
            if isinstance(edited_items, pd.DataFrame):
                 edited_items["Total"] = edited_items["Cantidad"] * edited_items["Precio Unitario"]
                 st.session_state.items = edited_items

        subtotal = st.session_state.items["Total"].sum()

        st.header("Resumen y Opciones")
        ...
        if submitted:
            ...
            elif edited_items.empty or edited_items["Descripción"].isnull().all():
